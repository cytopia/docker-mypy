...
_test-version:
	...
	@if [ "$(VERSION)" = "latest" ]; then \
		...
		LATEST="$$( \
			curl -Ss https://github.com/python/mypy/releases \
				| tac \
				| tac \
		...
		if ! docker run --rm $(IMAGE) --version | grep -E "^mypy $${LATEST}"; then \
			echo "Failed"; \
			exit 1; \
		fi; \
	else \
		echo "Testing for tag: $(VERSION)"; \
		if ! docker run --rm $(IMAGE) --version | grep -E "^mypy $(VERSION)"; then \
			echo "Failed"; \
			exit 1; \
		fi; \
	fi; \
